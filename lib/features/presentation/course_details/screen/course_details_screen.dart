              ...
              slivers: [
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        InstructorListTile(
                          ...
                          onPressed: () {
                            context.pushNamed(
                                RouteConstants.visitProfileScreenRoute,
                                arguments: course.instructorId);
                          },
                        ),
                        Row(
                          children: [
                ...
                SliverPadding(
                  ...
                  sliver: SliverList.separated(
                    ...
                    itemCount:
                        context.read<CourseDetailsCubit>().courseItems.length +
                            1,
                    itemBuilder: (context, index) {
                      ...
                      final item =
                          context.read<CourseDetailsCubit>().courseItems[index];
                      if (item is Video) {
                        final video = item;
                          ...
                          onTap: () {
                            if (course.isOwned) {
                              final items = context
                                  .read<CourseDetailsCubit>()
                                  .courseItems;
                              final videoIndex = items
                                  .indexWhere((item) => item.id == video.id);
                              MemoryCache.pushCurrentCourseItems(items);
                              MemoryCache.pushCurrentCourseItemIndex(
                                  videoIndex);
                              context.pushNamed(
                                  RouteConstants.courseVideoScreenRoute);
                            } else {
                              context.message(
                                  message:
                        ...
                        return CourseItemListTile(
                          title: '${index + 1}. ${assignment.title}',
                          ...
                          onTap: () {
                            if (course.isOwned) {
                              final items = context
                                  .read<CourseDetailsCubit>()
                                  .courseItems;
                              final assignmentIndex = items.indexWhere(
                                  (item) => item.id == assignment.id);
                              MemoryCache.pushCurrentCourseItems(items);
                              MemoryCache.pushCurrentCourseItemIndex(
                                  assignmentIndex);
                              context.pushNamed(
                                  RouteConstants.assignmentSolverScreenRoute,
