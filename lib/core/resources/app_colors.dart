import 'package:flutter/material.dart';

class AppColors {
  //* main colors
  static const Color primaryGradientColor_1 = Color(0xFFF4C049);
  static const Color primaryGradientColor_2 = Color(0xFFF4D692);
  //
  static const Color primaryColor = Color(0xFFF4A900);
  static Color fadedPrimaryColor = const Color(0xFFF4A900).withOpacity(0.24);
  static const Color inversePrimaryColor = Color(0xFF00D9BD);
  static const Color surfaceColor = Color(0xFF664950);
  static const Color onSurfaceColor = Color(0xFF402E32);
  static const Color whiteColor = Color(0xFFFFFFFF);
  //
  static const Color lightBackgroundColor = Color(0xFFF8E8C0);
  static const Color lightSecondaryColor = Color(0xFFF1DEB6);
  static const Color lightHintColor = Color(0xFFD9B68E);
  static const Color lightErrorColor = Color(0xFFD72638);
  //
  static const Color darkBackgroundColor = Color(0xFF1A1A1A);
  static const Color darkSecondaryColor = Color(0xFF242424);
  static const Color darkHintColor = Color(0xFF402E32);
  static const Color darkErrorColor = Color(0xFFC1121F);
}
